#!/usr/bin/env bash
# Build the wixels host + every widget plugin into ./build, then install the plugin
# dylibs next to the host executable (build/<config>/) where PluginLoader finds them
# at launch.
#
...
for dir in plugins/*/; do
    ...
    ( cd "$dir" && swift build -c "$CONFIG" --scratch-path build )
